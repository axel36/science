...
CALL TemplateForBlowUpDiagnostics(p_eff_ForEveryLayer,p_eff_ForParticularLayer,u,M_0,N_0,SGU)
    ...
    open(1,file = 'Results4.dat')
    open(2,file = 'Results_P4.dat')
    open(3,file = 'Results_testtestU.dat')
		...
		do s= 1,SGU-2
		    do i = 1,M_0
		        write(3,*) p_eff_ForEveryLayer(s,i)
		    enddo
	    enddo
    ...
    ! на определённом временном слое с номером m
		...
		DO n = 2,N_0
			DO s = 3,SGU
                ...
                p_eff_ForParticularLayer(s-2,n - 1,m)= ((Qlog(R_N/R_rN)))/(Qlog(r));
                if(s == SGU) then
                    if(m == 50) then
