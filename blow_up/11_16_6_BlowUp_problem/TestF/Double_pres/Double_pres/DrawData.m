...
hold on;
plot(t_0(1:M_0+1),Line,'-*k','MarkerSize',3);
...
for m = 1:M_0+1
    ...
    plot(x_0(1:N_0),p_eff_ForParticularLayer(S-2,1:N_0,m),'-sk','MarkerSize',5,'LineWidth',1);
    hold off;
    xlabel('x');
    ylabel('p^{eff}');
    title('p^{eff} for particular time layer');
    axis([x_0(2) x_0(length(x_0)-1) -2 4]);
    txt2=num2str(t_0(m));
    txt1='time =';
    txt=[txt1 ' ' txt2] ;
    hT = text(1, 3,txt); 
...
%     if m == 1
%         imwrite(img,cmap,'animationF.gif','gif','LoopCount',Inf,'DelayTime',0.1);
%     else
%         imwrite(img,cmap,'animationF.gif','gif','WriteMode','append','DelayTime',0.1);
%     end
% end
%-------------------------------------------------------------------------------
