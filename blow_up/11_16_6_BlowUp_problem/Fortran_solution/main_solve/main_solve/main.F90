...
program main
 use ieee_arithmetic
    ...
    r_t=2q0    ! По t
    SGU=12      !       $$$$__КОЛИЧЕСТВО СГУЩЕНИЙ__$$$$
...
CALL TemplateForBlowUpDiagnostics(p_eff_ForEveryLayer,p_eff_ForParticularLayer,u(1:M_0+1,1:N_0+1,1:SGU),M_0,N_0,SGU)
    ...
    open(1,file = 'ResultsGE.dat')
    open(2,file = 'Results_PGE.dat')
    open(3,file = 'Results_testtestGE.dat')
		write(1,*) N_0+1
		write(1,*) M_0+1
		write(1,*) SGU
		!write(1,*) x_left
...
real(16):: p_eff_ForEveryLayer(1:SGU-2,1:M_0),p_eff_ForParticularLayer(1:SGU-2,1:N_0+1,1:M_0+1)
